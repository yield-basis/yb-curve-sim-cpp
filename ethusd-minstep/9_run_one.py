...
detailed_data_file = Path(out_json_path.parent / 'detailed-output.json')
detailed_data_file = detailed_data_file.rename(detailed_data_file.parent / f"run-{NAME}-detailed.json")
with open(detailed_data_file) as f:
    data = json.load(f)
    with lzma.open(str(detailed_data_file) + '.xz', 'wt') as f_out:
